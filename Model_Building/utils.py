...
def get_collection_as_dataframe(database_name:str,collection_name:str)->pd.DataFrame:
    ...
    try:
        ...
        train_df = pd.DataFrame(list(mongo_client[database_name][collection_name].find()))
        logging.info(f"Found columns: {train_df.columns}")
        if "_id" in train_df.columns:
            logging.info(f"Dropping column: _id ")
            train_df = train_df.drop("_id",axis=1)
        logging.info(f"Row and columns in df: {train_df.shape}")
        return train_df
...
def save_object(file_path: str, obj: object) -> None:
    try:
        ...
        raise MartException(e, sys) from e
